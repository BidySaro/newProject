...
def getCalendarFor(year, month):
 ...
 # The blank rows have ten spaces in between the | day separators:
    ...
    currentDate = datetime.date(year, month, 1)
     # __________________________________________________________________________________________________DONT UNDERSTAND FROM HERE
...
# Check if we're done with the month:
        if currentDate.month != month:
            break
        #____________________________________________________________________________________________TO HERE
...
calendarFilename = 'calendrier_{}_{}.txt'.format(year, month)
with open(calendarFilename, 'w') as fileObj:
    fileObj.write(calText)
print('Enregistré dans ' + calendarFilename)